...
<body>
    ...
    <div id="start-screen">
        ...
        <div class="text-xs text-gray-300 max-w-md text-center">
            Стройте ферму, ставьте Королевские замки, украшайте территорию!<br>
        ...
        <div class="flex justify-between items-start pointer-events-auto w-full max-w-4xl mx-auto">
            <div class="glass-panel p-2 flex gap-3 w-full justify-between items-center">
                <div class="text-center min-w-[60px]">
                    <div class="text-[9px] text-gray-500 uppercase font-bold">Яйца</div>
                    <div class="text-lg font-bold text-yellow-600">🥚 <span id="money-display">250</span></div>
                </div>
                <div class="text-center min-w-[70px]">
                    <div class="text-[9px] text-gray-500 uppercase font-bold">Склад</div>
                    <div class="text-sm font-bold text-blue-600"><span id="storage-current">0</span> / <span id="capacity-display">150</span></div>
                </div>
                <div class="text-center min-w-[50px]">
                    <div class="text-[9px] text-gray-500 uppercase font-bold">Куры</div>
                    <div class="text-lg font-bold text-gray-700">🐔 <span id="chicken-display">0</span></div>
                </div>
                <div class="text-center min-w-[60px]">
                    <div class="text-[9px] text-gray-500 uppercase font-bold">Доход</div>
                    <div class="text-lg font-bold text-green-600">+<span id="income-display">0</span>/с</div>
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-center mb-1 pointer-events-auto w-full">
            <div class="glass-panel w-full max-w-2xl flex flex-col">
                ...
                <div class="tab-scroll" id="tabs-container">
                    <div class="tab-btn active" onclick="switchTab('production')">🏭 Курятники</div>
                    <div class="tab-btn" onclick="switchTab('resources')">🌾 Ресурсы</div>
                    <div class="tab-btn" onclick="switchTab('storage')">📦 Склады</div>
                    <div class="tab-btn" onclick="switchTab('nature')">🌲 Природа</div>
                    <div class="tab-btn" onclick="switchTab('village')">🏘️ Декор</div>
                </div>
                ...
                <div class="px-2 py-1 text-[10px] text-gray-400 bg-gray-50 rounded-b-lg flex justify-between items-center h-8">
                     <span class="truncate pr-2">Инфо: <span id="selected-desc" class="font-normal text-gray-500">Выберите здание</span></span>
                     <button onclick="selectBuilding('demolish')" class="bg-red-100 hover:bg-red-200 text-red-600 px-2 py-1 rounded text-xs font-bold transition">🚧 Снос</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script type="module">
        ...
        const buildingDb = {
            // --- Production (Coops) ---
            'coop':       { name: "Курятник", price: 50, icon: "🏠", type: 'coop', subType: 'basic', income: 1, capacity: 5, desc: "Базовый домик" },
            'coop_big':   { name: "Большой", price: 150, icon: "🏡", type: 'coop', subType: 'big', income: 1.5, capacity: 12, desc: "Больше места и кур" },
            'coop_mod':   { name: "Модерн", price: 250, icon: "🏢", type: 'coop', subType: 'modern', income: 2, capacity: 18, desc: "Стильный и вместительный" },
            'coop_gold':  { name: "Золотой", price: 800, icon: "💰", type: 'coop', subType: 'gold', income: 4, capacity: 25, desc: "Элитное жилье" },
            'coop_royal': { name: "Замок", price: 2500, icon: "🏰", type: 'coop', subType: 'royal', income: 10, capacity: 60, desc: "Королевский дворец!" },
            
            // --- Resources (Feeders) ---
            'feeder':     { name: "Кормушка", price: 30, icon: "🌽", type: 'feeder', subType: 'small', desc: "Радиус 5 клеток" },
            'trough':     { name: "Корыто", price: 70, icon: "🥖", type: 'feeder', subType: 'long', desc: "Радиус 8 клеток" },
            'water':      { name: "Поилка", price: 100, icon: "💧", type: 'feeder', subType: 'water', desc: "Нужна всем" },

            // --- Storage ---
            'storage':    { name: "Сарай", price: 100, icon: "🏚️", type: 'storage', subType: 'basic', capacity: 150, desc: "+150 яиц" },
            'barn':       { name: "Амбар", price: 300, icon: "🛖", type: 'storage', subType: 'barn', capacity: 500, desc: "+500 яиц" },
            'silo':       { name: "Силос", price: 600, icon: "🏭", type: 'storage', subType: 'silo', capacity: 1200, desc: "+1200 яиц" },

            // --- Nature ---
            'tree':       { name: "Дуб", price: 15, icon: "🌳", type: 'decor', subType: 'tree' },
            'pine':       { name: "Ель", price: 20, icon: "🌲", type: 'decor', subType: 'pine' },
            'bush':       { name: "Куст", price: 10, icon: "🌿", type: 'decor', subType: 'bush' },
            'flower_r':   { name: "Розы", price: 5, icon: "🌹", type: 'decor', subType: 'flower_r' },
            'flower_y':   { name: "Тюльпан", price: 5, icon: "🌻", type: 'decor', subType: 'flower_y' },
            'rock':       { name: "Камень", price: 8, icon: "🪨", type: 'decor', subType: 'rock' },

            // --- Village ---
            'fence':      { name: "Забор", price: 5, icon: "🪵", type: 'decor', subType: 'fence' },
            'fence_s':    { name: "Стена", price: 10, icon: "🧱", type: 'decor', subType: 'wall' },
            'lamp':       { name: "Фонарь", price: 25, icon: "💡", type: 'decor', subType: 'lamp' },
            'well':       { name: "Колодец", price: 150, icon: "🕳️", type: 'decor', subType: 'well' },
            'windmill':   { name: "Мельница", price: 500, icon: "🌬️", type: 'decor', subType: 'windmill', desc: "Вращается!" },
            'scarecrow':  { name: "Чучело", price: 40, icon: "🎃", type: 'decor', subType: 'scarecrow' }
        };
        ...
        window.selectBuilding = (id) => {
            ...
            if (id === 'demolish') {
                descEl.innerText = "Кликните по зданию для сноса (50% возврат)";
            ...
            if (id) {
                ...
                descEl.innerText = "Выберите здание";
            }
        };
